...
<%
   ...
   //장점 : 쉽다.
   //단점 : 분업화 , 유지보수 (코드 한곳에) >> 관심사의 분리 안되있다.
   ...
   >>java 웹 기반 기술 아니다 >> 계산기 , 그림판 , 윈도우 프로그램 >> 웹 (web,java) > 웹 전용  (request,response)
   >>java 페이지로 Web서비스 하자 >> login.java >> 단점 >> 배우기 어렵다 , 디자인 작업(협업 x)
   >>디자인 >>jsp탄생 >> 장점 >> UI 작업 >> Page >> Html기반 >>디자인 작업(o) >> 단점 JAVA <%안에 써야함
   >>혼재(java + UI)
   
   response.setContentType("text/html; charset=UTF-8");
   PrintWriter out = response.hetWriter();
   out.print("<html>");
   ...
   정리)
   서로의 장점을 살리자(현대적인 프로그래밍은 잘하는 것을 기반 , 업무툴) >>MVC 대두 ....
   ...
   서비스하기 위해서
   1. 공통점 : Apache Tomcat
   1.1 html : web server(apache)  :  클라이언트 요청 (ex : http://www.it.co.kr/login.html)
   >> web server >> 요청을 받아서 >> login.html 있다면 >>클라이언트에게 전달 (응답)
   
   2.jsp : was(tomcat)을 통해서 컴파일 >> class 파일 생성 >> 생성 파일 실행 (정적데이터 + 동적데이터)
   ...
   문제)상식 ASP, ASPX, PHP, JSP 공통적인 장점은 뭘까요?
         코드 노출이 안돼요.... >> 보안상 좋아요 >> 클라이언트는 실행된 결과만 보여요 (text)
         
   서버구성: default.jsp
   2022년 14:10 서버 오픈
   홍길동 최초요청자 -> http://www.it.or.kr/default.jsp -> was (처음, 기존 실행 된 적이 있는지)
...
<%
   ...
   2. 스크립트 요소
    2.1 스크립트 릿 <% java
    2.2 표현식 (출력방법) : 출력하는 대상(Client 발우져) : <%= response.write
    2.3 선언부 (공통자원: 공통함수: page )
   */
%>
