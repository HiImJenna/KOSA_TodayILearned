...
class Buyer2 {
	...
	Buyer2(int money, int bonuspoint) {
		// 초기화
		cart = new Product2[10]; // 카트 초기화 (Point)
		this.money = money;
	...
	void Buy(Product2 n) { // 함수의 parameter 제품객체의 주소를 받아서 ..(가격,포인트)
		...
		cart[index++] = n; // 0번째 index 담고 후치증가 index = 1 ...
		///////////////////////////////////////////////////////////
	...
	void summery() {
		...
		/*
		 * for (int i = 0; i < cart.length; i++) { //실제 물건이 없으면 null >> 연산 예외 //다행이도
		 * 10개를 샀으면 ... if(cart[i] == null) break; //랜덤이었으면 continue;로 totalprice +=
		 * cart[i].price; totalbonuspoint += cart[i].bonuspoint; productlist +=
		 * cart[i].toString() + " ";
		 */
